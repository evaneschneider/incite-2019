...
\begin{figure}[t]
    \centering
    \begin{minipage}{0.45\textwidth}
	\caption{  dM / dlogT from sims \label{fig:sim_phase}}
    \end{minipage}\hfill
    \begin{minipage}{0.55\textwidth}
...
As the discussion in the previous section makes clear, understanding the phase structure of turbulent, pressurized astrophysical flows is key to modeling galaxies.  It is also computationally challenging, with the ratio of the injection scale to the cooling length of order $R_{\rm CGM}/\ell_{\rm cool} \approx 10^3$.  In this section, we describe a systematic plan to model idealized flows which can model these flows under conditions appropriate for our physical system.  First, we need to determine what resolution is required to converge on the properties of interest (object RO.A in Table~\ref{table:RO}).  As detailed earlier, we believe we know what range of scales are important, but it is not clear precisely how many cells across the cooling length $l_{\rm cool}$ are necessary, and how sensitive the results are to the largest scales modeled ($R_{\rm CGM}$).  Therefore, to begin, we will identify a fiducial simulation (with parameters typical of the Milky Way CGM), and carry out a convergence study.  Once the convergence requirements are set, we will systematically vary our key parameters: the mean pressure in the box and the velocity dispersion (Research Object RO.B).  Finally, we will use these results to connect back to the global galaxy simulations (and their cosmological cousins) by developing a simple physical model and generating observational predictions (Research Object RO.C).
